...
const Checkout = () => {
  ...
  const validate = () => {
    ...
    if (paymentMethod === 'easypaisa' || paymentMethod === 'jazzcash') {
      ...
      else if (!/^03\d{9}$/.test(mobilePayment.accountNumber.replace(/\s/g, ''))) errs.accountNumber = 'Invalid Pakistani mobile number (03XXXXXXXXX)';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const orderData = {
        ...
        mobilePaymentInfo: ['easypaisa', 'jazzcash'].includes(paymentMethod) ? {
          ...mobilePayment,
          provider: paymentMethod === 'easypaisa' ? 'Easypaisa' : 'JazzCash',
        } : undefined,
      };
  ...
  return (
    <div className="min-h-screen bg-luxury-black pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          <Link to="/cart" className="inline-flex items-center gap-2 text-luxury-gray hover:text-luxury-gold text-sm transition-colors">
            <ArrowLeft className="w-4 h-4" /> Back to Cart
          </Link>
        </div>
        <h1 className="font-display text-4xl text-white mb-10">Checkout</h1>

        <div ref={formRef} className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2 space-y-6">
            <div className="luxury-card rounded-xl p-6 sm:p-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-8 h-8 rounded-full bg-luxury-gold text-luxury-black flex items-center justify-center text-sm font-bold">1</div>
                <h2 className="font-display text-xl text-white">Guest Information</h2>
              </div>
            ...
            <div className="luxury-card rounded-xl p-6 sm:p-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-8 h-8 rounded-full bg-luxury-gold text-luxury-black flex items-center justify-center text-sm font-bold">2</div>
                  ...
                  return (
                    <button
                      ...
                      className={`relative p-5 rounded-xl border-2 text-left transition-all ${
                        selected ? 'border-luxury-gold bg-luxury-gold/5' : 'border-luxury-gold/10 bg-luxury-card hover:border-luxury-gold/30'
                      }`}
                    >
                      {selected && (
                        <div className="absolute top-3 right-3 w-5 h-5 rounded-full bg-luxury-gold flex items-center justify-center">
              ...
              {paymentMethod === 'card' && (
                <div className="space-y-4 border-t border-luxury-gold/10 pt-6">
                  <div>
                    <label className="block text-sm text-luxury-gray mb-1.5">Card Number *</label>
                    <input name="number" value={card.number} onChange={handleCardChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="4242 4242 4242 4242" />
                    {errors.cardNumber && <p className="text-red-400 text-xs mt-1">{errors.cardNumber}</p>}
                  </div>
                  <div>
                    <label className="block text-sm text-luxury-gray mb-1.5">Cardholder Name *</label>
                    <input name="name" value={card.name} onChange={handleCardChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="John Doe" />
                    {errors.cardName && <p className="text-red-400 text-xs mt-1">{errors.cardName}</p>}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm text-luxury-gray mb-1.5">Expiry Date *</label>
                      <input name="expiry" value={card.expiry} onChange={handleCardChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="MM/YY" />
                      {errors.expiry && <p className="text-red-400 text-xs mt-1">{errors.expiry}</p>}
                    </div>
                    <div>
                      <label className="block text-sm text-luxury-gray mb-1.5">CVC *</label>
                      <input name="cvc" value={card.cvc} onChange={handleCardChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="123" />
                ...
                <div className="space-y-4 border-t border-luxury-gold/10 pt-6">
                  <div className="bg-luxury-black/50 rounded-lg p-4 mb-4">
                    <p className="text-sm text-luxury-gray">
                      Send payment to <span className="text-luxury-gold font-medium">{paymentMethod === 'easypaisa' ? 'Easypaisa' : 'JazzCash'}</span> account:
                    </p>
                    <p className="text-lg font-mono text-white mt-1">03XX-XXXXXXX</p>
                    <p className="text-xs text-luxury-gray mt-1">Then enter your details below</p>
                  </div>
                  <div>
                    <label className="block text-sm text-luxury-gray mb-1.5">Your {paymentMethod === 'easypaisa' ? 'Easypaisa' : 'JazzCash'} Account Number *</label>
                    <input name="accountNumber" value={mobilePayment.accountNumber} onChange={handleMobileChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="03XXXXXXXXX" />
                    {errors.accountNumber && <p className="text-red-400 text-xs mt-1">{errors.accountNumber}</p>}
                  </div>
                  <div>
                    <label className="block text-sm text-luxury-gray mb-1.5">Transaction ID *</label>
                    <input name="transactionId" value={mobilePayment.transactionId} onChange={handleMobileChange} className="input-luxury w-full rounded-lg px-4 py-3 text-sm" placeholder="Enter transaction/reference ID" />
              ...
              <button
                ...
                disabled={submitting}
                className="btn-primary w-full py-3.5 rounded-lg text-sm tracking-wider uppercase flex items-center justify-center gap-2 disabled:opacity-70 mt-6"
              >
          ...
          <div className="lg:col-span-1">
            <div className="luxury-card rounded-xl p-6 sticky top-28">
              ...
              <div className="space-y-3 mb-6">
                {items.map((item) => (
                  <div key={item.product} className="flex justify-between text-sm">
                    <span className="text-luxury-gray truncate mr-4">{item.name} × {item.quantity}</span>
                    <span className="text-white whitespace-nowrap">{formatPrice(item.price * item.quantity)}</span>
                  </div>
                ))}
              </div>
              <div className="border-t border-luxury-gold/10 pt-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-luxury-gray">Payment</span>
                  <span className="text-white capitalize">{paymentLabel[paymentMethod]}</span>
                </div>
              </div>
