...
const paymentLabels = { cod: 'COD', card: 'Card', easypaisa: 'Easypaisa', jazzcash: 'JazzCash' };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="hidden sm:flex gap-2">
          {statuses.map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs tracking-wider uppercase transition-all ${
                statusFilter === s ? 'bg-luxury-gold text-luxury-black' : 'bg-luxury-card text-luxury-gray hover:text-white border border-luxury-gold/10'
              }`}
            >
              {s || 'All'}
            </button>
        ...
        <select
          ...
          className="sm:hidden input-luxury rounded-lg px-3 py-2 text-xs"
        >
          {statuses.map((s) => (
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                orders.map((order) => (
                  <tr key={order._id} className="border-b border-luxury-gold/5 hover:bg-white/[0.02] transition-colors">
                    <td className="px-4 py-3"><span className="text-sm font-mono text-luxury-gold">{order.orderId}</span></td>
                    <td className="px-4 py-3">
                      <div>
                        <p className="text-sm text-white">{order.guestInfo?.fullName}</p>
                        <p className="text-xs text-luxury-gray">{order.guestInfo?.phoneNumber}</p>
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {order.cardInfo?.lastFour && <span className="text-xs text-luxury-gray-dark">•••• {order.cardInfo.lastFour}</span>}
                        {order.mobilePaymentInfo?.transactionId && <span className="text-xs text-luxury-gray-dark">ID: {order.mobilePaymentInfo.transactionId}</span>}
                      </div>
                      <span className={`text-[10px] mt-1 inline-block px-1.5 py-0.5 rounded ${order.paymentStatus === 'Paid' ? 'text-green-400 bg-green-400/10' : 'text-amber-400 bg-amber-400/10'}`}>
                        {order.paymentStatus}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-[10px] px-2 py-1 rounded-full uppercase tracking-wider font-medium border ${statusColors[order.status] || 'text-luxury-gray bg-luxury-gray/10'}`}>
                        {order.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <select
                        ...
                        className="input-luxury rounded-lg px-3 py-1.5 text-xs"
                      >
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white flex items-center gap-1.5">
                    {selectedOrder.paymentMethod === 'card' ? <CreditCard className="w-3.5 h-3.5 text-luxury-gold" /> : selectedOrder.paymentMethod === 'easypaisa' || selectedOrder.paymentMethod === 'jazzcash' ? <Smartphone className="w-3.5 h-3.5 text-luxury-gold" /> : <Banknote className="w-3.5 h-3.5 text-luxury-gold" />}
                    {paymentLabels[selectedOrder.paymentMethod]}
                    {selectedOrder.cardInfo?.lastFour && <span className="text-luxury-gray">•••• {selectedOrder.cardInfo.lastFour}</span>}
                    {selectedOrder.mobilePaymentInfo?.transactionId && <span className="text-luxury-gray text-xs">({selectedOrder.mobilePaymentInfo.transactionId})</span>}
                  </span>
                </div>
              <div className="border-t border-luxury-gold/10 pt-3 mt-3">
                {selectedOrder.items?.map((item, i) => (
                  <div key={i} className="flex justify-between py-1"><span className="text-luxury-gray">{item.name} × {item.quantity}</span><span className="text-white">{formatPrice(item.price * item.quantity)}</span></div>
                ))}
              </div>
              <div className="border-t border-luxury-gold/10 pt-3 flex justify-between">
                <span className="text-white font-medium">Total</span>
                <span className="text-luxury-gold font-display">{formatPrice(selectedOrder.total)}</span>
              </div>
            </div>
          </div>
        </div>
