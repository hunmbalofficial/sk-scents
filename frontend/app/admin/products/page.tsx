...
export default function AdminProductsPage() {
  ...
  const loadProducts = () => {
    setLoading(true);
    productService.getAll().then((res) => setProducts(res.data)).catch(() => {}).finally(() => setLoading(false));
  };

  const resetForm = () => {
    setForm({ name: '', price: '', discountPrice: '', description: '', notes: '', category: 'Oriental', gender: 'Unisex', stock: '10', topNotes: '', middleNotes: '', baseNotes: '', imageUrls: '', featured: false, bestSeller: false });
    setEditing(null);
    setShowForm(false);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editing) { await productService.update(editing, formData); addToast('Product updated', 'success'); }
      else { await productService.create(formData); addToast('Product created', 'success'); }
      resetForm();
      loadProducts();
    } catch (err: any) { addToast(err?.response?.data?.message || 'Failed to save product', 'error'); }
  };
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-luxury-card rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-luxury-gold/10">
            <div className="flex items-center justify-between mb-6"><h2 className="font-display text-lg text-white">{editing ? 'Edit Product' : 'Add Product'}</h2><button onClick={resetForm} className="text-luxury-gray hover:text-white"><X className="w-5 h-5" /></button></div>
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm text-luxury-gray mb-1">Name</label><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm" /></div>
                ...
                <div><label className="block text-sm text-luxury-gray mb-1">Category</label>
                  <select value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm">
                    {['Oriental', 'Woody', 'Fresh', 'Floral', 'Collections', 'Testers'].map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
                ...
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm text-luxury-gray mb-1">Description</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm min-h-[80px]" /></div>
                ...
                <div><label className="block text-sm text-luxury-gray mb-1">Top Notes</label><input value={form.topNotes} onChange={(e) => setForm({ ...form, topNotes: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm" /></div>
                <div><label className="block text-sm text-luxury-gray mb-1">Middle Notes</label><input value={form.middleNotes} onChange={(e) => setForm({ ...form, middleNotes: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm" /></div>
                <div><label className="block text-sm text-luxury-gray mb-1">Base Notes</label><input value={form.baseNotes} onChange={(e) => setForm({ ...form, baseNotes: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm" /></div>
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm text-luxury-gray mb-1">Upload Images</label><input type="file" multiple accept="image/*" className="text-sm text-luxury-gray file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:bg-luxury-gold/10 file:text-luxury-gold w-full" /></div>
                <div className="col-span-1 sm:col-span-2"><label className="block text-sm text-luxury-gray mb-1">Or paste image URLs</label><input value={form.imageUrls} onChange={(e) => setForm({ ...form, imageUrls: e.target.value })} className="input-luxury w-full rounded-lg px-4 py-2.5 text-sm" /></div>
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={handleSubmit} className="btn-primary flex-1 py-2.5 rounded-lg text-sm">{editing ? 'Update' : 'Create'} Product</button>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr><td colSpan={6} className="px-4 py-8 text-center text-luxury-gray text-sm">No products yet</td></tr>
              ) : products.map((product) => (
                <tr key={product._id} className="border-b border-luxury-gold/5 hover:bg-white/[0.02] transition-colors">
                  <td className="px-4 py-3"><div className="flex items-center gap-3"><img src={product.images?.[0] || ''} alt="" className="w-10 h-10 rounded-lg object-cover bg-luxury-black" /><span className="text-sm text-white">{product.name}</span></div></td>
