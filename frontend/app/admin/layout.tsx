    ...
    <div className="min-h-screen bg-luxury-black flex">
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-luxury-card border-r border-luxury-gold/10 transform transition-transform duration-300 lg:transform-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 border-b border-luxury-gold/10">
          <div className="flex items-center justify-between">
            <div><span className="font-display text-xl text-luxury-gold">SK</span><span className="font-display text-sm text-white/60 ml-1">Admin</span></div>
          ...
          {links.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => setSidebarOpen(false)}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all ${isActive(link.href, link.exact) ? 'bg-luxury-gold/10 text-luxury-gold border border-luxury-gold/20' : 'text-luxury-gray hover:text-white hover:bg-white/5'}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-luxury-gold/10">
          <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-luxury-gray hover:text-red-400 hover:bg-red-500/5 transition-all w-full">
      ...
      <div className="flex-1 min-w-0">
        <header className="sticky top-0 z-30 bg-luxury-black/80 backdrop-blur-xl border-b border-luxury-gold/10 px-4 sm:px-6 py-4">
          <div className="flex items-center justify-between">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden text-luxury-gray"><Menu className="w-5 h-5" /></button>
