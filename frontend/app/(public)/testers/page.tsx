...
export default function TestersPage() {
  ...
  useEffect(() => {
    ...
    if (sectionRef.current) gsap.fromTo(sectionRef.current, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.6, ease: 'power3.out' });
  }, []);

  const handleAdd = (product: any) => { addItem(product); addToast('Added to cart', 'success'); };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? Array.from({ length: 8 }).map((_, i) => (
            ...
            <div className="col-span-full text-center py-20">
              ...
              <Link href="/shop" className="btn-primary px-6 py-3 rounded-xl text-sm tracking-wider uppercase inline-flex items-center gap-2"><ShoppingBag className="w-4 h-4" /> Shop All</Link>
            </div>
            ...
            <div key={product._id} className="group luxury-card rounded-2xl overflow-hidden hover:border-luxury-gold/30 transition-all duration-500">
              <Link href={`/product/${product._id}`} className="block aspect-[4/5] sm:aspect-square overflow-hidden bg-luxury-card">
              ...
              <div className="p-3 sm:p-5">
                ...
                <Link href={`/product/${product._id}`}><h3 className="font-display text-sm sm:text-lg text-white group-hover:text-luxury-gold transition-colors mb-2">{product.name}</h3></Link>
                ...
                <button onClick={() => handleAdd(product)} className="w-full bg-luxury-gold/10 border border-luxury-gold/20 text-luxury-gold hover:bg-luxury-gold/20 rounded-xl py-2.5 sm:py-3 text-[10px] sm:text-xs tracking-wider uppercase transition-all duration-300 flex items-center justify-center gap-2"><ShoppingBag className="w-3.5 h-3.5" /> Add to Cart</button>
              </div>
            </div>
          ))}
        </div>
