...
interface Product { _id: string; name: string; price: number; discountPrice?: number; images?: string[]; gender?: string; stock?: number; }

export default function TestersSection() {
  ...
  useEffect(() => {
    productService.getAll({ category: 'Testers' }).then((res) => setProducts((res.data || []).slice(0, 4))).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  useEffect(() => {
    ...
    gsap.fromTo(cards, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 0.8, stagger: 0.12, ease: 'power3.out', scrollTrigger: { trigger: gridRef.current, start: 'top 80%' } });
  }, [products]);
    ...
    <section className="py-24 bg-luxury-card/50 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div ref={headingRef} className="flex flex-col sm:flex-row items-start sm:items-end justify-between mb-16 gap-4">
          ...
          <Link href="/testers" className="btn-outline px-6 py-2.5 rounded-lg text-sm tracking-wider uppercase flex items-center gap-2 group whitespace-nowrap shrink-0">
        ...
        <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {loading ? Array.from({ length: 4 }).map((_, i) => (
            ...
            <div key={product._id} className="tester-card group luxury-card rounded-2xl overflow-hidden hover:border-luxury-gold/30 transition-all duration-500">
              <Link href={`/product/${product._id}`} className="block aspect-[4/5] sm:aspect-square overflow-hidden bg-luxury-card relative">
                <img src={product.images?.[0] || ''} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                <span className="absolute top-2 left-2 bg-luxury-gold/90 text-luxury-black text-[9px] sm:text-[10px] uppercase tracking-wider font-semibold px-2 py-1 rounded">Tester</span>
              </Link>
              <div className="p-3 sm:p-5">
                <p className="text-[10px] sm:text-xs text-luxury-gray tracking-wider uppercase mb-1">{product.gender || 'Unisex'}</p>
                <Link href={`/product/${product._id}`}><h3 className="font-display text-sm sm:text-lg text-white group-hover:text-luxury-gold transition-colors mb-2 line-clamp-1">{product.name}</h3></Link>
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button onClick={(e) => handleAdd(e, product)} className="bg-luxury-gold/10 border border-luxury-gold/20 text-luxury-gold hover:bg-luxury-gold hover:text-luxury-black rounded-lg p-2 sm:px-3 sm:py-2 transition-all duration-300"><ShoppingBag className="w-3.5 h-3.5 sm:w-4 sm:h-4" /></button>
                </div>
              </div>
            </div>
          ))}
