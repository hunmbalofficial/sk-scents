...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(badgeRef.current, { opacity: 0, y: 20, scale: 0.9 }, { opacity: 1, y: 0, scale: 1, duration: 0.8 })
      ...
      .fromTo(ctaRef.current!.querySelectorAll('.cta-btn'), { opacity: 0, y: 20, scale: 0.9 }, { opacity: 1, y: 0, scale: 1, duration: 0.6, stagger: 0.12 }, '-=0.4')
      .fromTo(imageRef.current, { opacity: 0, scale: 0.7, rotate: -5 }, { opacity: 1, scale: 1, rotate: 0, duration: 1.4, ease: 'back.out(1.4)' }, '-=1.2')
      .fromTo(glowRef.current, { opacity: 0, scale: 0.4 }, { opacity: 1, scale: 1, duration: 1.8, ease: 'power2.out' }, '-=1.5')
      .fromTo(lineRef.current, { scaleY: 0 }, { scaleY: 1, duration: 1.2, ease: 'power3.out' }, '-=1.2');
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-center min-h-screen py-28 lg:py-0">
          <div className="lg:col-span-7 text-center lg:text-left">
            ...
            <div ref={ctaRef} className="flex flex-wrap gap-4 justify-center lg:justify-start">
              <Link href="/shop" className="cta-btn btn-primary px-8 py-4 rounded-full text-sm tracking-wider uppercase flex items-center gap-2 group font-medium relative overflow-hidden">
                <span className="relative z-10">Shop Collection</span>
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform relative z-10" />
              </Link>
              <Link href="/shop?featured=true" className="cta-btn btn-outline px-8 py-4 rounded-full text-sm tracking-wider uppercase font-medium group">
          ...
          <div className="lg:col-span-5 relative hidden lg:flex items-center justify-center min-h-[600px]">
            ...
            <div ref={imageRef} className="relative w-full h-full flex items-center justify-center">
              <div className="absolute top-8 right-0 w-56 h-72 rounded-2xl overflow-hidden border border-luxury-gold/20 shadow-2xl shadow-luxury-gold/10 z-10">
                <img src="https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=600&q=80" alt="Luxury Fragrance" className="w-full h-full object-cover" />
              ...
              <div className="absolute bottom-16 left-4 w-44 h-56 rounded-2xl overflow-hidden border border-luxury-gold/20 shadow-2xl shadow-luxury-gold/10 z-20">
                <img src="https://images.unsplash.com/photo-1541643600914-78b084683601?w=600&q=80" alt="Luxury Fragrance" className="w-full h-full object-cover" />
              </div>
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-80 rounded-3xl overflow-hidden border-2 border-luxury-gold/30 shadow-2xl shadow-luxury-gold/20 z-30 bg-luxury-black">
                <img src="https://images.unsplash.com/photo-1615634260167-c8cdede054de?w=800&q=80" alt="Premium Fragrance" className="w-full h-full object-cover" />
              </div>
              ...
              <div className="absolute -bottom-6 right-8 z-40 glass border border-luxury-gold/30 rounded-2xl px-4 py-3 backdrop-blur-xl">
                <div className="flex items-center gap-1.5 mb-1">
                  {[1, 2, 3, 4, 5].map((i) => (<Star key={i} className="w-3 h-3 fill-luxury-gold text-luxury-gold" />))}
                </div>
